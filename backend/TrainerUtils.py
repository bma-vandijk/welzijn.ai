...
def test(device,model,dataset,bi_direction):
    ...
    for i, data in enumerate(dataset.test_dataloader):
        ...
        with torch.no_grad():
            with autocast():
                predict, _ = model.forward(x.to(device),y.to(device),len_p,len_q,len_oi,trun_count,bi_direction)
                predicted_index = torch.argmax(predict,dim=1).cpu().detach().numpy()
                print(f"Dialog: {data['dialogue']}")
                print("\n")
                print(f"Question: {data['question']}")
                print("\n")
                print(f"Predicted choices: {[data['choice'][i] for i in predicted_index]}")
                print("\n")
                print("\n")
                print("\n")
                correct += numpy.sum(torch.argmax(predict,dim=1).cpu().detach().numpy()==y.cpu().detach().numpy())
...
def predict(device,model,dataset,bi_direction):
    model.encoder.eval()
    iters = 0
    for i, data in enumerate(dataset.test_dataloader):
        iters += 1
        ...
        with torch.no_grad():
            with autocast():
                predict, _ = model.forward(x.to(device),y.to(device),len_p,len_q,len_oi,trun_count,bi_direction)
                predicted_index = torch.argmax(predict,dim=1).cpu().detach().numpy()
                predicted = [data['choice'][i] for i in predicted_index]
