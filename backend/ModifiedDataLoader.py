...
class loader():
    ...
    def __init__(self,dataset_name,batch_size,new_tokens, chat, question, options):
        self.batch_size = batch_size
        self.name = dataset_name
        self.new_tokens = new_tokens
        
        '''Modify data collection sources of datasets'''    
        if self.name == 'ChatTest':
            self.call_encoder = self.dream_pre_processing
            self.dataset = [[],[],[]]
            data = {'id': 2040, 'dialogue_id': '3-145', 'dialogue': chat, 'question': question, 'choice': options, 'answer': options[0]} #Options[0] is standard answer to fit the required dataset format 
            self.dataset[2] = [data]
            self.opt_num = 3 
